...
const translateErr = (err) =>
  err === ERR_CANNOT_CONNECT
    ...
    : err === ERR_HASS_HOST_REQUIRED
      ? "Please enter a eDashboard URL."
      : err === ERR_INVALID_HTTPS_TO_HTTP
        ? html`
            Cannot connect to eDashboard instances over "http://".
            ${seeFAQ("https")}
          `
        : `Unknown error (${err}).`;
...
const INTRO = html`
  <p>
    eDashboard Cast allows you to cast your eDashboard installation to
...
export class HcConnect extends LitElement {
      ...
      return html`
        <hc-layout>
          <div class="card-content">
            ${INTRO}
            <p>
              To get started, enter your eDashboard URL and click authorize.
              If you want a preview instead, click the show demo button.
            </p>
            <ha-textfield
              label="eDashboard URL"
  ...
  private async _handleConnect() {
    ...
    if (value === "") {
      this.error = "Please enter a eDashboard URL.";
      return;
    }
    if (value.indexOf("://") === -1) {
      this.error =
